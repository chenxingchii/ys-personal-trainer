...
## 23. 当前实现进度与产品决策（2026-09-02）

当前代码已完成视频输入、MediaPipe Worker、整段视频分析、内部动作测量、视频质量门禁、教练式诊断报告、本机历史和复测入口。前端不展示原始角度、帧号、曲线或规则版本；这些数据只作为诊断生成和后续校准输入。

诊断报告固定输出一个主要建议，并用自然、通俗、专业教练口吻说明：看到了什么、可能影响什么、下一次怎么做以及推荐练习。质量检查未通过时不调用诊断规则，页面只提示具体重拍方法。

## 24. App/小程序与云端演进评估

后续封装成原生 App、微信小程序或其他客户端是可行的，但会增加以下工作，不纳入当前本地 MVP：

- 客户端需要适配视频选择、相机权限、MediaPipe/WASM 或原生姿态 SDK 的运行环境。
- 云端需要增加用户身份、鉴权、报告 API、对象存储、访问控制、失败重试和数据删除接口。
- 上传视频涉及隐私同意、传输加密、存储期限、撤回和删除流程；默认仍应优先上传派生报告，原始视频按用户明确授权处理。
- 本机报告存储应抽象为 `ReportRepository` 接口，当前实现使用浏览器本地存储，未来可替换为云端实现，不改变诊断规则和报告结构。
- 云端同步不会改变姿态分析的核心输入输出：客户端或服务端均应保留规则版本、模型版本和分析时间，保证报告可追溯。

推荐演进顺序：先完成真实视频校准和本机复测闭环，再抽象报告仓储接口，最后根据用户是否需要跨设备访问决定是否建设云端上传。这样不会因为提前建设后端而拖慢 MVP 验证。

## 25. Android 封装实施顺序（当前阶段）

当前优先目标是让 Android 手机可稳定使用，暂不处理 iOS 和微信小程序。实施顺序如下：

1. **PWA 真机验证**：使用 HTTPS 部署地址安装到 Android Chrome 主屏幕，验证拍摄、选视频、模型加载、报告和复测闭环。
2. **Android 兼容验收**：覆盖相机权限拒绝/重试、视频旋转元数据、常见 H.264/MP4 文件、低端设备内存和后台恢复。
3. **Capacitor 封装**：PWA 流程稳定后再加入 Capacitor Android 工程，生成 APK/AAB；原有 React、Worker 和分析规则保持复用。
4. **云端准备**：将本机报告实现收敛到 `ReportRepository` 接口，未来以云端实现替换，不改动诊断引擎。

局域网调试可运行 `pnpm dev:lan`，手机访问电脑局域网 IP；安装和离线能力必须在 HTTPS 地址验收。当前 Service Worker 只缓存同源页面和分析资源，原始视频与历史报告仍保存在当前设备。

后续优化应按“真机问题 → 性能 → 识别校准 → 产品能力 → 云端同步”的顺序推进，每次改动单独提交 Git commit，保留可回滚节点。

当前详细执行记录见 [[项目进度-Android演示版]]。该文件是下次恢复开发上下文的首要入口，包含最近 commit、模块定位、真机验收清单和常用命令。
