...
describe('动作角度与规则分析', () => {
  ...
  it('从姿态帧提取阶段指标并生成完成度与优先问题', () => {
    ...
    expect(analysis?.metrics.find((item) => item.id === 'pre-push-shin')?.value).toBeDefined()
    expect(analysis?.metrics.find((item) => item.id === 'pre-push-hip')?.value).toBeDefined()
    expect(analysis?.series[0]).toHaveProperty('leftShin')
