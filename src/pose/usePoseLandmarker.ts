...
function waitForSeek(video: HTMLVideoElement, targetTimeMs: number) {
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    const onError = () => { cleanup(); reject(new Error('视频定位失败，请重试或更换视频。')) }
    const cleanup = () => { video.removeEventListener('seeked', onSeeked); video.removeEventListener('error', onError) }
    video.addEventListener('seeked', onSeeked, { once: true })
    video.addEventListener('error', onError, { once: true })
    try { video.currentTime = targetTimeMs / 1000 } catch { cleanup(); reject(new Error('视频定位失败，请重试或更换视频。')) }
  })
}
...
export function usePoseLandmarker() {
  ...
  const disposeWorker = useCallback((updateState: boolean) => {
  ...
  const handleMessage = useCallback((message: PoseWorkerResponse) => {
    const batchIsActive = analysisRef.current?.active === true
    if (message.type === 'loading') {
      setState((current) => ({ ...current, phase: 'loading', loadingStage: message.stage, error: undefined }))
      return
    }
    if (message.type === 'ready') {
      setState((current) => ({ ...current, phase: batchIsActive ? 'analyzing' : 'ready', delegate: message.delegate, modelVariant: message.modelVariant, loadTimeMs: message.loadTimeMs, fallbackReason: message.fallbackReason, error: undefined }))
    ...
    if (message.type === 'result') {
      ...
      setState((current) => ({ ...current, phase: batchIsActive ? 'analyzing' : 'success', frame: message.frame, error: undefined }))
      return
    }
    if (message.type === 'no-pose') {
      pendingRef.current.get(message.requestId)?.resolve(null)
      pendingRef.current.delete(message.requestId)
      setState((current) => ({ ...current, phase: batchIsActive ? 'analyzing' : 'no-pose', frame: batchIsActive ? current.frame : undefined, error: undefined }))
      return
    }
  ...
  const ensureWorker = useCallback(() => {
    ...
    const promise = new Promise<void>((resolve, reject) => { resolveInitialization = resolve; rejectInitialization = reject })
  ...
  const requestFrame = useCallback(async (video: HTMLVideoElement, session: number, frameIndex: number) => {
    ...
    if (session !== sessionRef.current) { image.close(); return null }
    requestIndexRef.current += 1
    const requestId = `pose-${session}-${requestIndexRef.current}`
    const message: PoseWorkerRequest = { type: 'analyze-frame', requestId, frameIndex, mediaTimeMs: video.currentTime * 1000, imageWidth: video.videoWidth, imageHeight: video.videoHeight, image }
    const worker = workerRef.current
    if (!worker) { image.close(); throw new Error('姿态识别服务不可用，请刷新页面后重试。') }
    return new Promise<PoseFrame | null>((resolve, reject) => {
      pendingRef.current.set(requestId, { resolve, reject })
      try { worker.postMessage(message, [image]) } catch (error) {
  ...
  const analyzeFrame = useCallback(async (video: HTMLVideoElement) => {
    ...
    setState((current) => ({ ...current, phase: 'analyzing', frame: undefined, frames: [], error: undefined }))
    return requestFrame(video, session, 0)
  }, [ensureWorker, requestFrame])

  const analyzeVideo = useCallback(async (video: HTMLVideoElement, sampleIntervalMs = DEFAULT_SAMPLE_INTERVAL_MS) => {
    if (!isVideoReady(video)) {
      const message = '视频画面尚未准备好，请等待画面显示后重试。'
      setState((current) => ({ ...current, phase: 'error', error: message }))
      throw new Error(message)
    ...
    const updateProgress = (completed: boolean) => {
      ...
      setState((current) => ({ ...current, phase: completed ? (frames.length ? 'success' : 'no-pose') : 'analyzing', frame: frames.at(-1), frames: [...frames], analysis: next, error: undefined }))
    }

    try {
      if (targets.length > 0) {
        setState((current) => ({ ...current, phase: 'analyzing', frame: undefined, frames: [], analysis: { processedFrames: 0, totalFrames, progress: 0, completed: false }, error: undefined }))
        ...
        setState((current) => ({ ...current, phase: 'analyzing', frame: undefined, frames: [], analysis: { processedFrames: 0, completed: false }, error: undefined }))
        let lastSampleMs = -Infinity
        await video.play()
        await new Promise<void>((resolve, reject) => {
          let timer: ReturnType<typeof setTimeout> | undefined
          const cleanup = () => { if (timer) clearTimeout(timer); video.removeEventListener('ended', onEnded); video.removeEventListener('error', onError) }
          const onEnded = () => { cleanup(); resolve() }
          const onError = () => { cleanup(); reject(new Error('视频播放失败，无法完成整段分析。')) }
          const tick = async () => {
            if (session !== sessionRef.current || analysisRef.current !== analysis) { cleanup(); resolve(); return }
      ...
      if (session === sessionRef.current && analysisRef.current === analysis) {
        ...
        setState((current) => ({ ...current, phase: 'error', error: message, analysis: { processedFrames, totalFrames, progress: totalFrames ? processedFrames / totalFrames : undefined, completed: false } }))
        throw error
      }
      return []
    } finally {
  ...
  const clearResult = useCallback(() => {
    setState((current) => {
      ...
      return { ...current, phase: workerRef.current ? 'ready' : 'idle', frame: undefined, frames: [], analysis: undefined, error: undefined }
    })
  }, [])
